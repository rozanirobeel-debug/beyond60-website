...
export function Experience() {
  useEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      // scroll-scrubbed drone sequence — video currentTime is driven directly by scroll progress within a pinned range, not autoplay.
      // desktop only: mobile/iOS Safari can't reliably seek video on scroll, so those viewports show the static final-frame fallback instead (see the isDesktop=false branch below).
      const scrubOnUpdate = (video: HTMLVideoElement | null) => (self: ScrollTrigger) => {
      ...
      if (isDesktop) {
        ...
        locationVideo?.pause()
        gsap.timeline({
          scrollTrigger: { trigger: '.location-section', start: 'top top', end: '+=140%', scrub: true, pin: true, onUpdate: scrubOnUpdate(locationVideo) },
        ...
        scenicVideo?.pause()
        ScrollTrigger.create({ trigger: '.scenic-section', start: 'top top', end: '+=160%', scrub: true, pin: true, onUpdate: scrubOnUpdate(scenicVideo) })
      } else {
