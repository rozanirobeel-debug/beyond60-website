...
export function Experience() {
  useEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      // amenity story cards — direction alternates with the media/text layout swap, plus continuous internal parallax on desktop so each card keeps moving through its own scroll range
      gsap.utils.toArray<HTMLElement>('.amenity-story').forEach((item, i) => {
        ...
        const dir = i % 2 === 0 ? 1 : -1
        const drift = 30 + (i % 3) * 14
        const tl = gsap.timeline({ scrollTrigger: isDesktop ? { trigger: item, start: 'top 85%', end: 'bottom top', scrub: 1 } : { trigger: item, start: 'top 85%', once: true } })
        if (media) tl.fromTo(media, { autoAlpha: 0, xPercent: dir * 8, scale: 1.12 }, { autoAlpha: 1, xPercent: 0, scale: isDesktop ? 1.05 : 1, duration: .35, ease: easeOut }, 0)
        if (body) tl.fromTo(body, { autoAlpha: 0, xPercent: dir * -6 }, { autoAlpha: 1, xPercent: 0, duration: .35, ease: easeOut }, .05)
        if (isDesktop && media) tl.to(media, { y: -drift, scale: 1, ease: 'none' }, .35)
      })
  ...
  return <main id="top">
    ...
    <section className="care-section section" id="care" aria-labelledby="care-heading"><BotanicalMotif className="motif motif-right" /><div className="shell"><p className="eyebrow" data-reveal>Thoughtful support</p><h2 className="display-heading" id="care-heading" data-reveal>{specializedCare.intro}</h2><div className="care-grid">{specializedCare.items.map((item) => { const Icon = careIcons[item.icon as keyof typeof careIcons]; return <Card className="care-card" key={item.title}><CardBody><Icon size={24} /><h3>{item.title}</h3><p>{item.detail}</p></CardBody></Card> })}</div><p className="trust-line" data-reveal>{specializedCare.closing}</p></div></section>
    <section className="amenities-section section" id="amenities" aria-labelledby="amenities-heading"><BotanicalMotif className="motif motif-amenities" /><div className="shell amenities-intro" data-reveal><p className="eyebrow">Daily life</p><h2 className="display-heading" id="amenities-heading">Spaces made for living, not just staying.</h2></div><div className="amenities-stack shell">{amenities.map((item, index) => <Card className="amenity-story" key={item.title}><CardMedia><img src={item.image} alt={item.alt} loading={index > 1 ? 'lazy' : 'eager'} /><span className="amenity-tag">{item.label}</span></CardMedia><CardBody><span>{String(index + 1).padStart(2, '0')} / {item.label}</span><h3>{item.title}</h3><p>{item.detail}</p></CardBody></Card>)}</div></section>
